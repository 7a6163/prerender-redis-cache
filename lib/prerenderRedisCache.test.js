...
describe('prerender-redis-cache', () => {
    ...
    describe('Compression (real Redis)', () => {
        // Self-contained: redisClient is only created in afterEach, so these
        // must not depend on a previous test having run.
        let raw;

        beforeEach(async () => {
            raw = redis.createClient({ url: 'redis://127.0.0.1:6379/15' })
                .withTypeMapping({ [redis.RESP_TYPES.BLOB_STRING]: Buffer });
            await raw.connect();
        });
        ...
        it('stores zstd bytes that survive the wire and read back intact', async () => {
            const zlib = require('zlib');
            const content = '<html>héllo ☃ 中文 ' + 'x'.repeat(5000) + '</html>';
            const url = 'http://example.com/compressed';

            await plugin.pageLoaded({
                prerender: { url, statusCode: 200, content, headers: { 'content-type': 'text/html' } }
            }, mockRes, jest.fn());

            // Read the value back as bytes, bypassing the plugin entirely
            const bytes = await raw.get('example.com/compressed');

            expect([...bytes.subarray(0, 4)]).toEqual([0x28, 0xb5, 0x2f, 0xfd]);
            expect(bytes.length).toBeLessThan(Buffer.byteLength(content) / 4);
            expect(JSON.parse(zlib.zstdDecompressSync(bytes).toString()).content).toBe(content);

            // And the plugin serves it back byte-for-byte
            const res = { send: jest.fn(), setHeader: jest.fn() };
            await plugin.requestReceived({ method: 'GET', prerender: { url } }, res, jest.fn());
            expect(res.send).toHaveBeenCalledWith(200, content);
        });

        it.each([
            ['gzip, as written by 1.2.x', (json) => require('zlib').gzipSync(json)],
            ['plain JSON, as written before 1.2', (json) => Buffer.from(json)]
        ])('still serves entries stored as %s', async (_label, encode) => {
            const url = 'http://example.com/legacy';
            const body = JSON.stringify({
                statusCode: 200, content: '<html>legacy</html>', headers: {}
            });
            await raw.set('example.com/legacy', encode(body));

            const res = { send: jest.fn(), setHeader: jest.fn() };
            await plugin.requestReceived({ method: 'GET', prerender: { url } }, res, jest.fn());

            expect(res.send).toHaveBeenCalledWith(200, '<html>legacy</html>');
        });
    });

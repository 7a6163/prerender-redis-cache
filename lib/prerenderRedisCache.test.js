...
describe('prerender-redis-cache', () => {
    ...
    describe('Compression (real Redis)', () => {
        ...
        it('stores gzipped bytes that survive the wire and read back intact', async () => {
            ...
            expect(JSON.parse(zlib.gunzipSync(bytes).toString()).content).toBe(content);
        ...
        it('still serves entries written before compression existed', async () => {
            const url = 'http://example.com/legacy';
            await raw.set('example.com/legacy', JSON.stringify({

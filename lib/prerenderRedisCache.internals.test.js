...
/** What a cache entry looks like on the wire, in either format. */
const stored = (obj) => Buffer.from(JSON.stringify(obj));
const storedGzip = (obj) => zlib.gzipSync(JSON.stringify(obj));
...
const makeFakeClient = () => {
    ...
    client.withTypeMapping = jest.fn(() => client);
...
const load = ({ env = {}, client = makeFakeClient() } = {}) => {
    ...
    delete process.env.PAGE_COMPRESS;
    ...
    jest.doMock('redis', () => ({
        createClient,
        RESP_TYPES: jest.requireActual('redis').RESP_TYPES
    }));
...
describe('prerenderRedisCache internals', () => {
    ...
    describe('Connection lifecycle', () => {
        ...
        it('serves from cache once ready and bypasses again on error', async () => {
            ...
            client.get.mockResolvedValue(storedGzip({
                statusCode: 200,
                content: 'cached',
                headers: {}
            }));
    ...
    describe('Bodyless responses', () => {
        it.each([204, 301, 410])('caches status %i with no content instead of throwing', async (statusCode) => {
            ...
            const payload = JSON.parse(zlib.zstdDecompressSync(client.set.mock.calls[0][1]).toString());
            expect(payload).toEqual({ statusCode, content: '', headers: {} });
        });
    });
    ...
    describe('Compression', () => {
        ...
        const cache = async (content) => {
            await plugin.pageLoaded({
                prerender: { url: 'http://a/b', statusCode: 200, content, headers: { 'content-type': 'text/html' } }
            }, makeRes(), jest.fn());
            return client.set.mock.calls[0][1];
        };

        it('reads through a Buffer view so gzip bytes are not UTF-8 mangled', () => {
            expect(client.withTypeMapping).toHaveBeenCalledWith({
                [require('redis').RESP_TYPES.BLOB_STRING]: Buffer
            });
        });

        it('stores pages zstd-compressed', async () => {
            const payload = await cache('<html>hello</html>');

            expect(Buffer.isBuffer(payload)).toBe(true);
            expect([...payload.subarray(0, 4)]).toEqual([0x28, 0xb5, 0x2f, 0xfd]);
            expect(JSON.parse(zlib.zstdDecompressSync(payload).toString())).toEqual({
                statusCode: 200,
        ...
        it('compresses harder than zstd\'s default level', async () => {
            // Varied markup, where level 9 measurably beats the default.
            let page = '<html><body>';
            for (let i = 0; i < 200; i++) {
                page += `<div class="item-${i}" data-id="${i * 7919}"><h2>Section ${i}</h2>` +
                    `<p>Lorem ipsum dolor sit ${i % 13} amet consectetur ${i % 7}.</p></div>`;
            }
            page += '</body></html>';

            const payload = await cache(page);

            // Same async API as the plugin, so the only difference is the level.
            // (The sync API frames output a byte differently - not comparable.)
            const json = JSON.stringify({
                statusCode: 200, content: page, headers: { 'content-type': 'text/html' }
            });
            const atDefaultLevel = await promisify(zlib.zstdCompress)(json, {});

            expect(payload.length).toBeLessThan(atDefaultLevel.length);
        });

        it('actually shrinks a realistic page', async () => {
            const page = `<html><body>${'<p>Lorem ipsum dolor sit amet.</p>'.repeat(100)}</body></html>`;
            const payload = await cache(page);

            expect(payload.length).toBeLessThan(page.length / 4);
            expect([...payload.subarray(0, 4)]).toEqual([0x28, 0xb5, 0x2f, 0xfd]);
        });

        it('survives a multi-byte round trip', async () => {
            const page = '<html>héllo ☃ 中文</html>';
            client.get.mockResolvedValue(await cache(page));
            const res = makeRes();

            await plugin.requestReceived({ method: 'GET', prerender: { url: 'http://a/b' } }, res, jest.fn());

            expect(res.send).toHaveBeenCalledWith(200, page);
        });

        it('still reads gzipped entries written by 1.2.x', async () => {
            // Writes are zstd now, but the cache must not be stranded on upgrade.
            client.get.mockResolvedValue(storedGzip({ statusCode: 200, content: 'gzip era', headers: {} }));
            const res = makeRes();

            await plugin.requestReceived({ method: 'GET', prerender: { url: 'http://a/b' } }, res, jest.fn());

            expect(res.send).toHaveBeenCalledWith(200, 'gzip era');
        });

        it('treats a corrupt zstd entry as a miss', async () => {
            client.get.mockResolvedValue(Buffer.concat([
                Buffer.from([0x28, 0xb5, 0x2f, 0xfd]), Buffer.from('garbage')
            ]));
            const next = jest.fn();
        ...
        it('reads an entry with only part of the zstd magic as plain', async () => {
            client.get.mockResolvedValue(Buffer.from([0x28, 0xb5, 0x2f, 0x00, 0x7b]));
            const next = jest.fn();

            await plugin.requestReceived({ method: 'GET', prerender: { url: 'http://a/b' } }, makeRes(), next);

            // Parsed as text and failed there, not handed to zstd
            expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Failed to parse'));
            expect(next).toHaveBeenCalled();
        });

        it('still reads uncompressed entries written before 1.2', async () => {
            // The cache migrates itself: old entries stay readable until they expire.
            client.get.mockResolvedValue(stored({ statusCode: 200, content: 'legacy', headers: {} }));
            const res = makeRes();

            await plugin.requestReceived({ method: 'GET', prerender: { url: 'http://a/b' } }, res, jest.fn());

            expect(res.send).toHaveBeenCalledWith(200, 'legacy');
        });

        it.each(['0', 'false', 'off', 'no', 'FALSE', 'Off'])(
            'writes plain JSON when PAGE_COMPRESS=%s', async (value) => {
                ({ plugin, client } = load({ env: { PAGE_COMPRESS: value } }));
                client.emit('ready');

                const payload = await cache('<html>hello</html>');

                expect(typeof payload).toBe('string');
                expect(JSON.parse(payload).content).toBe('<html>hello</html>');
            });

        it.each(['1', 'true', 'yes', 'handoff', 'nope', 'offline', '10', '', undefined])(
            'still compresses when PAGE_COMPRESS=%s', async (value) => {
                const env = value === undefined ? {} : { PAGE_COMPRESS: value };
                ({ plugin, client } = load({ env }));
                client.emit('ready');
        ...
        it('still reads gzipped entries after compression is turned off', async () => {
            // Flipping the flag must not strand what is already cached.
            ({ plugin, client } = load({ env: { PAGE_COMPRESS: '0' } }));
            client.emit('ready');
            client.get.mockResolvedValue(storedGzip({ statusCode: 200, content: 'old', headers: {} }));
            const res = makeRes();

            await plugin.requestReceived({ method: 'GET', prerender: { url: 'http://a/b' } }, res, jest.fn());

            expect(res.send).toHaveBeenCalledWith(200, 'old');
        });

        it('treats a corrupt gzip entry as a miss', async () => {
            client.get.mockResolvedValue(Buffer.from([0x1f, 0x8b, 0x08, 0x00, 0xff, 0xff]));
            const next = jest.fn();
        ...
        it.each([
            ...
            ['only the first magic byte', Buffer.from([0x1f, 0x00, 0x7b])],
            ['only the second magic byte', Buffer.from([0x7b, 0x8b])]
        ])('reads an entry with %s as plain, not gzip', async (_label, value) => {
            client.get.mockResolvedValue(value);
            const next = jest.fn();

            await plugin.requestReceived({ method: 'GET', prerender: { url: 'http://a/b' } }, makeRes(), next);

            // Parsed as text and failed there - not handed to gunzip, which
            // would throw a zlib error and log nothing.
            expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Failed to parse'));
            expect(next).toHaveBeenCalled();
        });
    });
    ...
    describe('TTL handling', () => {
        it('sets an expiry when PAGE_TTL is positive', async () => {
            ...
            expect(client.set).toHaveBeenCalledWith('a/b', expect.any(Buffer), { EX: 3600 });
        });
        ...
        it('stores without an expiry when PAGE_TTL is 0', async () => {
            ...
            expect(client.set).toHaveBeenCalledWith('a/b', expect.any(Buffer));
        });
        ...
        it('defaults to one day when PAGE_TTL is unset', async () => {
            ...
            expect(client.set).toHaveBeenCalledWith('a/b', expect.any(Buffer), { EX: 86400 });
        });
    ...
    describe('Cache reads', () => {
        ...
        it('treats an empty stored value as a miss rather than parsing it', async () => {
            client.get.mockResolvedValue(Buffer.alloc(0));
            const next = jest.fn();
        ...
        it('falls through when the cached entry is corrupt', async () => {
            client.get.mockResolvedValue(Buffer.from('{not json'));
            const next = jest.fn();

...
/**
 * Detect gzip by its magic bytes (1f 8b).
 * Entries written before compression was introduced are plain JSON, so the
 * cache migrates itself: old entries stay readable and age out via TTL.
 * @param {Buffer} buf - The raw cached value
 * @returns {Boolean} - True if the value is gzipped
 */
const isGzipped = (buf) => buf[0] === 0x1f && buf[1] === 0x8b;
...
const handleCacheGet = async (req, res, next) => {
    ...
    try {
        ...
        const json = isGzipped(cached) ? (await gunzip(cached)).toString() : cached.toString();
...
module.exports = {
    ...
    pageLoaded: async (req, res, next) => {
        ...
        try {
            ...
            const payload = COMPRESS ? await gzip(json) : json;

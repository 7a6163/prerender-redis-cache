...
client.on('ready', () => {
    ...
    if (connection.auth) {
        client.auth(connection.auth.split(':')[1], (error) => {
            ...
            // Select Redis database after authentication
            client.select(connection.database, (selectError) => {
                if (selectError) {
                    console.error(`Redis Cache Database Selection Error: ${selectError}`);
                    return;
                }
                redisOnline = true;
                reconnectAttempts = 0;
        ...
        // Select Redis database directly if no auth

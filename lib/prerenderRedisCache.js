...
// Level 9 measured smaller *and* faster to write than gzip -6 on real pages
// (react.dev/learn: 34KB in 1.65ms vs gzip's 42KB in 2.36ms).
const ZSTD_LEVEL = 9;
...
/**
 * Whether to gzip entries on write (default on; set PAGE_COMPRESS to 0/false/
 * off/no to disable). Reads always auto-detect the format, so this can be
 * flipped either way without invalidating anything already cached.
 * @returns {Boolean} - True if writes should be compressed
 */
const COMPRESS_DISABLED_VALUES = ['0', 'false', 'off', 'no'];
const isCompressionEnabled = () =>
    !COMPRESS_DISABLED_VALUES.includes(String(process.env.PAGE_COMPRESS).toLowerCase());
...
const COMPRESS = isCompressionEnabled();
...
// A Buffer view over the same connection. Cached pages are gzipped, so reads
// must not be UTF-8 decoded; writes go through the normal client.
const bufferClient = client.withTypeMapping({ [redis.RESP_TYPES.BLOB_STRING]: Buffer });
...
/**
 * Decode a cached value by sniffing its magic bytes.
 * Writes are always zstd, but older entries are gzip (1.2.x) or plain JSON
 * (pre-1.2), so the cache migrates itself: everything already stored stays
 * readable and ages out via TTL.
 * @param {Buffer} buf - The raw cached value
 * @returns {Promise<String>} - The decoded JSON text
 */
const decode = async (buf) => {
    if (ZSTD_MAGIC.equals(buf.subarray(0, 4))) {
        return (await zstdDecompress(buf)).toString();
    }
    if (buf[0] === 0x1f && buf[1] === 0x8b) {
        return (await gunzip(buf)).toString();
    }
    return buf.toString();
};
...
const handleCacheGet = async (req, res, next) => {
    ...
    try {
        const cached = await bufferClient.get(cacheKey);

        // A Buffer is always truthy, so an empty value needs an explicit check
        if (!cached || cached.length === 0) {
            return next();
        }

        const response = JSON.parse(await decode(cached));
...
module.exports = {
    ...
    pageLoaded: async (req, res, next) => {
        ...
        try {
            ...
            // Rendered HTML gzips to roughly a sixth of its size, and Redis
            // memory is the binding cost of a cache. Async so the compression
            // runs on the threadpool instead of blocking every other request.
            const json = JSON.stringify(response);
            const payload = COMPRESS
                ? await zstdCompress(json, { params: { [zlib.constants.ZSTD_c_compressionLevel]: ZSTD_LEVEL } })
                : json;
